...
void MainMenuScreen::onEnter()
{
    ...
    m_current = Vector2{static_cast<float>(canvas.width()) * 0.5f,
                        static_cast<float>(canvas.height()) * 0.62f};
    m_previous = m_current;
    m_velocity = Vector2{74.0f, 41.0f};  // vitesses non entieres : l'absence
                                         // d'interpolation se verrait
}
...
void MainMenuScreen::render(float alpha)
{
    ...
    DrawText(title, (canvas.width() - MeasureText(title, titleSize)) / 2, 34, titleSize,
             Color{226, 205, 154, 255});

    const char* hint = "F1 debug   -   ESC quitter";
    const int hintSize = 10;
    DrawText(hint, (canvas.width() - MeasureText(hint, hintSize)) / 2, 62, hintSize,
             Color{120, 110, 130, 255});

    // Rebond interpole : lisse meme quand le rendu tourne plus vite que 60 Hz.
    const Vector2 drawn{
        m_previous.x + (m_current.x - m_previous.x) * alpha,
