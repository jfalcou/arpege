    ...
    , m_height(height)
{
    ...
    // Pixel art : jamais de filtrage lineaire.
...
{
    // Hauteur source negative : les render textures d'OpenGL sont a l'envers.
...
Vector2 PixelCanvas::screenToCanvas(Vector2 screenPosition) const
{
    ...
    const float factor = static_cast<float>(scale());
    return Vector2{
        (screenPosition.x - dest.x) / factor,
        (screenPosition.y - dest.y) / factor
    };
}
