...
{
  TTS_EQUAL(arpg::integerScale(320, 180, 1280, 720), 4);
  TTS_EQUAL(arpg::integerScale(320, 180, 1920, 1080), 6);

  // Limited by the tightest axis, here the height.
  TTS_EQUAL(arpg::integerScale(320, 180, 1920, 720), 4);
};

TTS_CASE("Integer scale never drops below one")
{
  TTS_EQUAL(arpg::integerScale(320, 180, 160, 90), 1);
  TTS_EQUAL(arpg::integerScale(320, 180, 0, 0), 1);
};

TTS_CASE("Integer scale tolerates a degenerate canvas")
{
  TTS_EQUAL(arpg::integerScale(0, 0, 1280, 720), 1);
  TTS_EQUAL(arpg::integerScale(-320, -180, 1280, 720), 1);
};

TTS_CASE("An exactly fitting window leaves no letterbox")
{
  const auto rect = arpg::canvasDestination(320, 180, 1280, 720);
...
TTS_CASE("The canvas is centered inside a larger window")
{
  ...
  const auto rect = arpg::canvasDestination(320, 180, 1400, 800);
...
{
  const auto origin = arpg::windowToCanvas(arpg::ViewportPoint{0.0f, 0.0f}, 320, 180, 1280, 720);
  TTS_EQUAL(origin.x, 0.0f);
  TTS_EQUAL(origin.y, 0.0f);

  const auto middle = arpg::windowToCanvas(arpg::ViewportPoint{640.0f, 360.0f}, 320, 180, 1280, 720);
...
TTS_CASE("A letterboxed position maps outside the canvas")
{
  ...
  const auto point = arpg::windowToCanvas(arpg::ViewportPoint{10.0f, 40.0f}, 320, 180, 1400, 800);
...
{
  const auto rect = arpg::canvasDestination(320, 180, 1400, 800);
  const auto corner = arpg::windowToCanvas(arpg::ViewportPoint{rect.x, rect.y}, 320, 180, 1400, 800);
