...
void ScreenManager::applyPending()
{
    ...
    // Une commande peut en produire d'autres (un onEnter qui empile une
    // transition) : on travaille sur une copie et les nouvelles demandes
    // seront traitees a la frame suivante.
...
void ScreenManager::update(float dt)
{
    ...
    // On descend depuis le sommet tant que les ecrans laissent passer.
...
void ScreenManager::render(float alpha)
{
    ...
    // On repart du dernier ecran opaque : inutile de dessiner ce qu'il cache.
    std::size_t first = m_stack.size() - 1;
